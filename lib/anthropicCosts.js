...
export async function fetchAnthropicSpendUsd({ startingAt, endingAt }) {
  ...
  try {
    // Anthropic paginates the cost_report endpoint. With bucket_width=1d
    // and a long date range (we use 2024-01-01 → today, ~600+ days), the
    // response can span multiple pages. Follow the `next_page` cursor
    // until exhausted so we sum the FULL lifetime, not just the first
    // page's worth.
    let totalUsd = 0;
    ...
    let pageCount = 0;
    let nextPage = null;
    const MAX_PAGES = 20; // hard cap so a runaway response can't lock us
    ...
    do {
      ...
      // Response shape: { data: [{ starting_at, ending_at, results: [{ amount, currency, ... }] }, ...], has_more, next_page }
      // Anthropic returns `amount` as a STRING in the JSON. Number() parses it correctly.
