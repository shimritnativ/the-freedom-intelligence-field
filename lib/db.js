...
export async function recordDayCompletion({
  ...
  //
  // Why the UPDATE runs unconditionally (no `EXISTS (SELECT 1 FROM inserted)`
  // guard): when a day_completions row was inserted via a different code path
  // (manual SQL, an older code revision, a backfill before this CTE existed,
  // or a demo endpoint that wrote last_completed_day directly), the conflict
  // branch fires and the user pointer never catches up. We saw Saskia stuck
  // at last_completed_day = 2 with a Day 3 row in day_completions. Running
  // the UPDATE every time, using GREATEST, is idempotent and cheap and keeps
  // the two sources of truth aligned forever.
  const { rows } = await sql`
    WITH inserted AS (
    ...
    advanced AS (
      ...
      SET last_completed_day = GREATEST(COALESCE(last_completed_day, 0), ${day}),
          pitch_eligible = (COALESCE(last_completed_day, 0) >= 3 OR ${day} >= 3),
          updated_at = NOW()
      WHERE id = ${userId}
        AND COALESCE(last_completed_day, 0) < ${day}
      RETURNING id
    )
 ...
 *
 * Two-source unlock — WHICHEVER COMES FIRST wins:
 *   1. Time: 24h after first_login per day (Day 2 at +24h, Day 3 at +48h)
 *   2. Completion: if last_completed_day >= dayNum-1, the day is unlocked
 *      immediately. So a participant who completes Day 1 in hour 2 doesn't
 *      have to wait 22 hours for Day 2 — they can move on right away.
 *
 * Completion is set either by the "Complete Day X" button OR by the
 * structured-output auto-detector (lib/dayExtraction.js).
 */
export function isDayUnlocked(user, dayNum) {
  // Workshop tier: all three steps are open immediately from purchase.
  // No 24h wait, no completion gating. The Power Reset is delivered as
  // Steps so members can move through at their own pace before the
  // workshop begins.
  if (user && user.tier === "workshop") return true;
  // Completion path: prior day already done → unlocked
  const lastCompleted = Number(user && user.last_completed_day || 0);
  if (lastCompleted >= dayNum - 1) return true;
  // Time path: 24h * (dayNum - 1) elapsed since first login
 ...
 *   - For preview tier: return the highest day unlocked by TIME. Day 1 is
 *     immediate, Day 2 opens 24h after first_login, Day 3 opens 48h after
 *     first_login. We do NOT cap by current_day (last_completed_day + 1)
 *     because that would punish anyone whose Day 2 completion didn't
 *     auto-register: they'd be stuck on Day 2 forever even though the 48h
 *     mark had passed (Anne Bork case, Jun 22 2026). Geo's stated rule:
 *     "each day opens 24h after the first one, regardless of completion."
...
export function resolveActiveDay(user) {
  ...
  if (user.tier === "preview") {
    ...
    // Active day = MIN of:
    //   1. user.current_day (= last_completed_day + 1, capped at 3) — the
    //      day they should be WORKING ON based on what they've finished
    //   2. highestUnlockedDay — the highest day TIME has unlocked
    //
    // Previously this returned only highestUnlockedDay, treating time as
    // pure progression. That broke the wrong-day guardrail: a user whose
    // first_login was >48h ago always read as "on Day 3" regardless of
    // whether they'd actually completed Day 1, so pasting the Day 1 prompt
    // got blocked with "you're already on Day 3, paste Day 3 instead" and
    // they'd loop forever (Katrine Kovacs hit this on 2026-06-30).
    //
    // The MIN means: time UNLOCKS days in the library (the prompts become
    // visible) but completion gates what day you're ACTIVELY working on.
    // If you stalled on Day 1 for a week, you're still on Day 1 until you
    // click "Complete Day 1 →" to advance manually.
    return Math.min(
      Number(user.current_day) || 1,
      highestUnlockedDay(user)
    );
  }
  if (user.tier === "full") {
  ...
  if (user.tier === "workshop") {
    // Workshop tier gets the Reset in Steps with NO time gate. All three
    // are unlocked immediately from purchase moment. The active step is
    // whatever they were last working on, bounded 1..3. Access closes on
    // workshop_expires_at (Oct 2 for the Sep 2026 workshop).
    if (user.workshop_expires_at && new Date(user.workshop_expires_at) < new Date()) {
      return null; // workshop access window has closed
    }
    return Math.min(user.current_day || 1, 3);
  }
  return null;
 ...
 */
export async function grantEntitlement({ email, tier, kajabiMemberId, subscriptionPlan, utm }) {
  const now = new Date();
  const ends = new Date(now.getTime() + 72 * 60 * 60 * 1000);
  ...
  const plan = (subscriptionPlan === "monthly" || subscriptionPlan === "yearly")
    ? subscriptionPlan
    : null;
  // First-touch UTM attribution. Passed in from the Zapier webhook (which
  // gets them from ThriveCart's checkout URL params, which got them from
  // the GHL landing page's link-rewriter). We use COALESCE(users.utm_*,
  // EXCLUDED.utm_*) in the conflict clause to keep the FIRST tracked
  // source if the user already has one — a returning buyer's second
  // purchase shouldn't overwrite the campaign that originally brought
  // them. Coerce empty strings to NULL so Zapier substituting blank
  // values doesn't wipe existing data.
  const utmSource   = (utm && utm.source   && String(utm.source).trim())   || null;
  const utmMedium   = (utm && utm.medium   && String(utm.medium).trim())   || null;
  const utmCampaign = (utm && utm.campaign && String(utm.campaign).trim()) || null;
  const utmContent  = (utm && utm.content  && String(utm.content).trim())  || null;
  const utmTerm     = (utm && utm.term     && String(utm.term).trim())     || null;
  const { rows } = await sql`
    ...
    ON CONFLICT (email) DO UPDATE SET
      ...
      -- First-touch UTM attribution: keep the EARLIEST source, never
      -- overwrite. If the existing row has utm_source set, leave it; only
      -- fill in NULL columns from the new grant.
      ...
      -- Returning-Kajabi-customer fix: when an existing row gets its first
      -- Field entitlement (the previous kajabi_entitled was false or null —
      -- meaning the row was created by some other MYP product months ago
      -- and is only now joining The Field), bump created_at to NOW so the
      -- launch-date dashboard filter counts them in the current window.
      -- If they already had a Field entitlement (e.g. they're upgrading
      -- Reset → Unlimited), leave their original Field join date intact.
      created_at = CASE
        WHEN users.kajabi_entitled IS NOT TRUE THEN NOW()
...
/**
 * Grant workshop-tier Field access. Used by the ThriveCart webhook when
 * a €27 VIP upgrade is purchased for "All The Way To The Top & Beyond"
 * (product 153).
 *
 * Behavior:
 * - Creates or updates the user with tier='workshop', kajabi_entitled=true.
 * - Sets workshop_expires_at to the passed expiration (Oct 2, 2026 for
 *   the September 2026 workshop).
 * - NEVER downgrades a full-tier user. If the account already has
 *   Unlimited, we leave it alone (the buyer already has better access).
 * - First-touch UTM attribution preserved via COALESCE like grantEntitlement.
 *
 * Returns the resulting users row.
 */
export async function grantWorkshopEntitlement({ email, expiresAt, utm }) {
  const utmSource   = (utm && utm.source   && String(utm.source).trim())   || null;
  const utmMedium   = (utm && utm.medium   && String(utm.medium).trim())   || null;
  const utmCampaign = (utm && utm.campaign && String(utm.campaign).trim()) || null;
  const utmContent  = (utm && utm.content  && String(utm.content).trim())  || null;
  const utmTerm     = (utm && utm.term     && String(utm.term).trim())     || null;
  const expIso = expiresAt instanceof Date ? expiresAt.toISOString() : String(expiresAt);
  const { rows } = await sql`
    INSERT INTO users (email, tier, kajabi_entitled, workshop_expires_at,
                       utm_source, utm_medium, utm_campaign, utm_content, utm_term)
    VALUES (${String(email).toLowerCase().trim()}, 'workshop'::user_tier, true,
            ${expIso}::timestamptz,
            ${utmSource}, ${utmMedium}, ${utmCampaign}, ${utmContent}, ${utmTerm})
    ON CONFLICT (email) DO UPDATE SET
      -- Never downgrade: if this account already has full Unlimited, keep
      -- it. Otherwise promote them to workshop tier. The ::user_tier casts
      -- on both branches of the CASE are non-optional — Postgres refuses
      -- to write bare text literals into an enum column and returns
      -- "column tier is of type user_tier but expression is of type
      -- text". This bit Geo during the first ATWT VIP purchase test
      -- (Aug 2026): the row was recorded but the tier grant failed
      -- silently on the ON CONFLICT path.
      tier = CASE WHEN users.tier = 'full' THEN 'full'::user_tier
                  ELSE 'workshop'::user_tier END,
      kajabi_entitled = true,
      -- Only set the expiry when we are actually granting workshop tier.
      workshop_expires_at = CASE WHEN users.tier = 'full' THEN users.workshop_expires_at
                                 ELSE ${expIso}::timestamptz END,
      utm_source   = COALESCE(users.utm_source,   EXCLUDED.utm_source),
      utm_medium   = COALESCE(users.utm_medium,   EXCLUDED.utm_medium),
      utm_campaign = COALESCE(users.utm_campaign, EXCLUDED.utm_campaign),
      utm_content  = COALESCE(users.utm_content,  EXCLUDED.utm_content),
      utm_term     = COALESCE(users.utm_term,     EXCLUDED.utm_term),
      -- First-time-Field flag same as grantEntitlement so launch dashboards
      -- pick them up in the current window.
      created_at = CASE
        WHEN users.kajabi_entitled IS NOT TRUE THEN NOW()
        ELSE users.created_at
      END,
