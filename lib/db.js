 ...
 *   - Otherwise return min(current_day, highestUnlockedDay) — i.e. cap the
 *     user's day at the highest progressively-unlocked day. This handles the
 *     edge case where someone finishes Day 1 in an hour (current_day = 2)
 *     but Day 2 has not unlocked yet — they're still on Day 1.
 *   - Full-tier users get day 3's prompt by default for now.
 */
...
export async function revokeEntitlementByEmail(email) {
  ...
  // cancellation event for the monthly product roughly the same instant the
  // yearly purchase grant lands. Without this guard, the cancellation Zap
  // would call us with revoke=1 and downgrade a customer who just paid for
  // yearly access.
  //
  // The check: if there is an activate:full webhook event for this email
  // within the last 5 minutes, skip the revoke. The activation already won;
