...
// Markers expanded over time as we saw real production outputs slip past
// the original strict-caps list. Detection is case-insensitive (see
// looksLikeDayCompletion lowercasing both sides), so any casing variant
// of these phrases will match. Bias: prefer false positives (cheap
// re-extraction) over false negatives (missed completion → stuck user
// like Anne Bork, Jun 22 2026).
...
export async function extractDayCompletion(text, day) {
  ...
  // is_completion was previously the gate. Removed: in practice Haiku rejects
  // legitimate completions when the production output blends Day 2/3 themes
  // (e.g., AI emits "72 HOUR POWER RESET — COMPLETE" wrap-up during a
  // day_at_send=2 session). The heuristic already requires day-specific
  // markers in the message, which is a strong enough gate on its own.
...
export async function maybeRecordDayCompletion({
  ...
  try {
    ...
    // Try to extract structured fields, but record the completion either way.
    // If Haiku fails (network, rate limit, schema rejection), we still want
    // the funnel to show the completion based on the heuristic match.
    let data = {};
    try {
      const extracted = await extractDayCompletion(assistantMessage, day);
      if (extracted) data = extracted;
    } catch (err) {
      console.warn("extract_failed_recording_with_empty_data", {
