...
export async function loadUserMemory(userId) {
  ...
  return renderMemoryBlock({ profile, completions, facts, processHistory });
}
...
function renderMemoryBlock({ profile, completions, facts, processHistory }) {
  ...
  return [
    ...
    "",
    "This is silent context loaded from every prior chat and completion. " +
      "Reference it naturally when relevant — call them by name, recall " +
      "what they decided in a past process, build on their declared " +
      "actions. NEVER recite this block back verbatim or list out their " +
      "facts as a summary. The participant cannot see this — to them it " +
      "feels like the Field simply remembers.",
    "",
    sections.join("\n\n"),
  ].join("\n");
}
